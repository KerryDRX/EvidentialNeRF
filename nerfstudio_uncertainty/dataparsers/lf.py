...
@dataclass
class LFConfig(NerfstudioDataParserConfig):
    _target: Type = field(default_factory=lambda: LF)
    """Target class to instantiate."""
    scene: Literal["africa", "basket", "statue", "torch",] = "africa"
    """Scene name."""
...
class LF(Nerfstudio):
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if self.config.scene == "africa":
            i_train = np.arange(5, 14, 2)
            i_eval = np.arange(6, 14, 2)
            self.config.scene_scale = 36.
        elif self.config.scene == "basket":
            i_train = np.arange(43, 50, 2)
            i_eval = np.arange(42, 50, 2)
            self.config.scene_scale = 50.
        elif self.config.scene == "statue":
            i_train = np.arange(67, 76, 2)
            i_eval = np.arange(68, 76, 2)
            self.config.scene_scale = 50.
        elif self.config.scene == "torch":
            i_train = np.arange(8, 17, 2)
            i_eval = np.arange(9, 17, 2)
